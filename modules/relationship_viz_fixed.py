...
def generate_network_graph(data, relationship_dir):
    ...
    # Base size with minimum dimensions, scale up as nodes increase
    width = max(20, 15 + (total_nodes / 10))
    height = max(12, 8 + (total_nodes / 15))
    ...
    # Calculate the actual horizontal positions for each layer
    layer_positions = {0: [], 1: [], 2: [], 3: [], 4: []}
    for node in G.nodes():
        layer = G.nodes[node]['layer']
        if layer in layer_positions:
            layer_positions[layer].append(pos[node][0])
    
    # Add column headers at the calculated positions
    header_labels = ["Programs", "Products", "Material Systems", "Suppliers", "Funding"]
    
    for layer in range(5):  # 5 layers: 0-4
        if layer in layer_positions and layer_positions[layer]:
            # Use the average position of nodes in this layer
            avg_pos = sum(layer_positions[layer]) / len(layer_positions[layer])
            plt.text(
                avg_pos, 
                1.05, 
                header_labels[layer], 
            ...
            # If no nodes in this layer, use evenly spaced positions
            fallback_pos = layer / 4
            plt.text(
                fallback_pos, 
                1.05, 
                header_labels[layer], 
    ...
    # Add a grid background to separate the columns - use calculated positions
    for layer in range(1, 5):
        if layer-1 in layer_positions and layer in layer_positions and layer_positions[layer-1] and layer_positions[layer]:
            # Calculate the midpoint between adjacent layers
            left_pos = max(layer_positions[layer-1])
            right_pos = min(layer_positions[layer])
            midpoint = (left_pos + right_pos) / 2
            plt.axvline(x=midpoint, color='lightgray', linestyle='--', alpha=0.5)
